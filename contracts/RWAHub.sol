...
import {IPricerReader} from "contracts/interfaces/IPricerReader.sol";
...
import {IRWAHub} from "contracts/interfaces/IRWAHub.sol";
...
abstract contract RWAHub is IRWAHub, ReentrancyGuard, AccessControlEnumerable {
  ...
  address public constant assetRecipient = 0xF67416a2C49f6A46FEe1c47681C5a3832cf8856c; // OUSG Investments CB Account
    ...
    checkRestrictions(msg.sender)
  {
    ...
    depositIdToDepositor[depositId] =
      Depositor({user: msg.sender, amountDepositedMinusFees: depositAmountAfterFee, priceId: 0});
   ...
   *      claiming a mint
   */
  function claimMint(bytes32[] calldata depositIds)
    external
    virtual
    nonReentrant
    ifNotPaused(subscriptionPaused)
    ...
    ifNotPaused(redemptionPaused)
  {
    ...
    redemptionIdToRedeemer[redemptionId] =
      Redeemer({user: msg.sender, amountRwaTokenBurned: amount, priceId: 0});
   ...
   */
  function claimRedemption(bytes32[] calldata redemptionIds)
    external
    virtual
    nonReentrant
    ifNotPaused(redemptionPaused)
  ...
  function addProof(
    ...
    depositIdToDepositor[txHash] =
      Depositor({user: user, amountDepositedMinusFees: depositAmountAfterFee, priceId: 0});
   ...
   */
  function setPriceIdForDeposits(bytes32[] calldata depositIds, uint256[] calldata priceIds)
    public
    virtual
    onlyRole(PRICE_ID_SETTER_ROLE)
  {
    uint256 depositsSize = depositIds.length;
   ...
   */
  function setPriceIdForRedemptions(bytes32[] calldata redemptionIds, uint256[] calldata priceIds)
    public
    virtual
    onlyRole(PRICE_ID_SETTER_ROLE)
  {
  ...
  function overwriteDepositor(
    ...
    depositIdToDepositor[depositIdToOverwrite] =
      Depositor({user: user, amountDepositedMinusFees: depositAmountAfterFee, priceId: priceId});
  ...
  function overwriteRedeemer(
    ...
    Redeemer memory oldRedeemer = redemptionIdToRedeemer[redemptionIdToOverwrite];
    redemptionIdToRedeemer[redemptionIdToOverwrite] =
      Redeemer({user: user, amountRwaTokenBurned: rwaTokenAmountBurned, priceId: priceId});
   ...
   *                                 redemption request
   */
  function setMinimumRedemptionAmount(uint256 _minimumRedemptionAmount)
    external
   ...
   *                         request
   */
  function setMinimumDepositAmount(uint256 minDepositAmount) external onlyRole(MANAGER_ADMIN) {
   ...
   */
  function setFeeRecipient(address newFeeRecipient) external onlyRole(MANAGER_ADMIN) {
   ...
   */
  function setAssetSender(address newAssetSender) external onlyRole(MANAGER_ADMIN) {
   ...
   */
  function _getMintFees(uint256 collateralAmount) internal view returns (uint256) {
   ...
   *                         (in decimals of `collateral`)
   */
  function _getRedemptionFees(uint256 collateralAmount) internal view returns (uint256) {
   ...
   */
  function _getMintAmountForPrice(uint256 depositAmt, uint256 price)
    internal
    view
   ...
   */
  function _getRedemptionAmountForRwa(uint256 rwaTokenAmountBurned, uint256 price)
    internal
    view
    returns (uint256 collateralOwed)
  {
