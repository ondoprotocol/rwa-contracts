...
import {RWAHub} from "contracts/RWAHub.sol";
import {IRWAHubOffChainRedemptions} from "contracts/interfaces/IRWAHubOffChainRedemptions.sol";
...
abstract contract RWAHubOffChainRedemptions is RWAHub, IRWAHubOffChainRedemptions {
  ...
  function requestRedemptionServicedOffchain(
    ...
    emit RedemptionRequestedServicedOffChain(
      msg.sender, redemptionId, amountRWATokenToRedeem, offChainDestination
    );
  }
   ...
   *                                         redemption amount
   */
  function setOffChainRedemptionMinimum(uint256 _minimumOffChainRedemptionAmount)
    external
    onlyRole(MANAGER_ADMIN)
  {
    ...
    emit OffChainRedemptionMinimumSet(oldMinimum, _minimumOffChainRedemptionAmount);
  }
}
