...
} from "contracts/usdy/restrictedUSDYMetadata/IRestrictedUSDYMetadata.sol";

contract RestrictedUSDYMetadata is IRestrictedUSDYMetadata, AccessControlEnumerable {
  bytes32 public constant RESTRICTED_LIST_SETTER = keccak256("RESTRICTED_LIST_SETTER");
  ...
  function addToRestrictedList(address account, Restriction calldata restriction)
    external
    onlyRole(RESTRICTED_LIST_SETTER)
  {
  ...
  function removeFromRestrictedList(address account, Restriction calldata restriction)
    external
    onlyRole(RESTRICTED_LIST_SETTER)
  {
    ...
    for (uint256 i = 0; i < restrictionList[account].length; i++) {
      if (
        restrictionList[account][i].depositId == restriction.depositId
          && restrictionList[account][i].amountRestricted == restriction.amountRestricted
          && restrictionList[account][i].restrictedUntil == restriction.restrictedUntil
      ) {
        restrictionRemoved = restrictionList[account][i];
        restrictionList[account][i] = restrictionList[account][restrictionList[account].length - 1];
  ...
  function getRestrictedAmount(address account) external view returns (uint256) {
