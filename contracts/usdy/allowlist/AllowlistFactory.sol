...
import {AllowlistProxy} from "contracts/usdy/allowlist/AllowlistProxy.sol";
import {AllowlistUpgradeable} from "contracts/usdy/allowlist/AllowlistUpgradeable.sol";
...
contract AllowlistFactory is IMulticall {
   ...
   *
   */
  function deployAllowlist(address admin, address setter)
    external
    onlyGuardian
    returns (address, address, address)
  {
    ...
    allowlistProxy =
      new AllowlistProxy(address(allowlistImplementation), address(allowlistProxyAdmin), "");
    AllowlistUpgradeable allowlistProxied = AllowlistUpgradeable(address(allowlistProxy));
    ...
    emit AllowlistDeployed(
      address(allowlistProxied), address(allowlistProxyAdmin), address(allowlistImplementation)
    );
    return
      (address(allowlistProxied), address(allowlistProxyAdmin), address(allowlistImplementation));
  }
   ...
   */
  event AllowlistDeployed(address proxy, address proxyAdmin, address implementation);

  modifier onlyGuardian() {
    require(msg.sender == GUARDIAN, "AllowlistFactory: You are not the Guardian");
    _;
  }
}
