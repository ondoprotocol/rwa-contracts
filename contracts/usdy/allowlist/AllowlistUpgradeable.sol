...
import {IAllowlist} from "contracts/usdy/allowlist/IAllowlist.sol";
import {
  Initializable
} from "contracts/external/openzeppelin/contracts-upgradeable/proxy/Initializable.sol";
import {
  AddressUpgradeable
} from "contracts/external/openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol";
import {
 ...
 */
contract AllowlistUpgradeable is Initializable, AccessControlEnumerableUpgradeable, IAllowlist {
   ...
   *      of the terms array
   */
  function setCurrentTermIndex(uint256 _currentTermIndex)
    public
    override
    onlyRole(ALLOWLIST_ADMIN)
  {
   ...
   */
  function setValidTermIndexes(uint256[] calldata _validIndexes)
    external
    override
    onlyRole(ALLOWLIST_ADMIN)
  {
   ...
   */
  function setAccountStatus(address account, uint256 termIndex, bool status)
    external
    override
    onlyRole(ALLOWLIST_SETTER)
  {
   ...
   */
  function addAccountToAllowlist(uint256 termIndex, address account, uint8 v, bytes32 r, bytes32 s)
    external
    override
  {
    ...
    bytes32 hashedMessage = ECDSA.toEthSignedMessageHash(bytes(terms[termIndex]));
   ...
   * @param status    New status of the account
   */
  function _setAccountStatus(address account, uint256 termIndex, bool status) internal {
