...
import {USDYbsa} from "contracts/usdy/usdy_w_blocklist_sanctions_and_allowlist/USDYbsa.sol";
 ...
 */
contract USDYbsa_Factory is IMulticall {
  ...
  USDYbsa public usdyImplementation;
   ...
   *
   */
  function deployUSDY(string calldata name, string calldata ticker, USDYListData calldata listData)
    external
    onlyGuardian
    returns (address, address, address)
  {
    usdyImplementation = new USDYbsa();
    usdyProxyAdmin = new ProxyAdmin();
    usdyProxy = new TokenProxy(address(usdyImplementation), address(usdyProxyAdmin), "");
    USDYbsa usdyProxied = USDYbsa(address(usdyProxy));
