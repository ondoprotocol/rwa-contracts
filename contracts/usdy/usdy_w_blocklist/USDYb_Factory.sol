...
import {USDYb} from "contracts/usdy/usdy_w_blocklist/USDYb.sol";
 ...
 */
contract USDYb_Factory is IMulticall {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  bytes32 public constant DEFAULT_ADMIN_ROLE = bytes32(0);

  address internal immutable GUARDIAN;
  USDYb public usdyImplementation;
   ...
   *
   */
  function deployUSDY(string calldata name, string calldata ticker, address blocklist)
    external
    onlyGuardian
    returns (address, address, address)
  {
    require(!initialized, "USDY_Factory: USDY already deployed");
    usdyImplementation = new USDYb();
    usdyProxyAdmin = new ProxyAdmin();
    usdyProxy = new TokenProxy(address(usdyImplementation), address(usdyProxyAdmin), "");
    USDYb usdyProxied = USDYb(address(usdyProxy));
    ...
    return (address(usdyProxied), address(usdyProxyAdmin), address(usdyImplementation));
  }
