...
import {USDYbs} from "contracts/usdy/usdy_w_blocklist_and_sanctions/USDYbs.sol";
 ...
 */
contract USDYbs_Factory is IMulticall {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  bytes32 public constant DEFAULT_ADMIN_ROLE = bytes32(0);

  address internal immutable GUARDIAN;
  USDYbs public usdyImplementation;
  ...
  function deployUSDY(
    ...
    usdyImplementation = new USDYbs();
    usdyProxyAdmin = new ProxyAdmin();
    usdyProxy = new TokenProxy(address(usdyImplementation), address(usdyProxyAdmin), "");
    USDYbs usdyProxied = USDYbs(address(usdyProxy));
    usdyProxied.initialize(name, ticker, blocklist, sanctionsList);
